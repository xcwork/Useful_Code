...
//   InverseCumulatives.h

#ifndef INVERSE_CUMULATIVES_H
#define INVERSE_CUMULATIVES_H

#define CQP(v) (P1+v*(P2+v*(P3+v*(P4+(P5+P6*v)*v))))
#define CQQ(v) (Q1+v*(Q2+v*(Q3+v*(Q4+(Q5+Q6*v)*v))))

template <typename T>
struct inverseCumulativeShawBrickman
{
__host__ __device__
...
T operator()(const T& u) const 
{
 ...
 template <typename T>
struct inverseCumulativeShawBrickmanUnsignedInt
{
__host__ __device__
