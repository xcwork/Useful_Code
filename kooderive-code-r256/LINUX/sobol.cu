...
int SobDevice(int n_vectors, int n_dimensions,unsigned path_offset,  thrust::device_vector<float>& d_output_vec, bool doInverseNormal, bool verbose)
{
    thrust::device_vector<unsigned int> intSobols(n_dimensions*n_vectors);
    int res= SobDeviceInts(n_vectors,  n_dimensions, path_offset,  intSobols);
    
    if (doInverseNormal)
    {
        inverseCumulativeShawBrickmanUnsignedInt<float> inv;
        thrust::transform(intSobols.begin(),intSobols.end(), d_output_vec.begin(),inv);
    }
    else
    {  toUniformFloat inv;
        thrust::transform(intSobols.begin(),intSobols.end(), d_output_vec.begin(),inv);
...
int SobDeviceInts(int n_vectors, int n_dimensions,unsigned path_offset,  thrust::device_vector<unsigned int>& d_output_device)
{
    ...
    CUT_CHECK_ERR("start of GPU allocation direction numbers\n");

    
    thrust::device_vector<unsigned int> dev_directions(n_dimensions * n_directions);
    unsigned int* d_directions= thrust::raw_pointer_cast(&*dev_directions.begin()); 

     // Allocate memory for the arrays
    std::vector<unsigned int> h_directions_vec(n_dimensions * n_directions);
    unsigned int *h_directions = &h_directions_vec[0]; 
    ...
    d_output_device.resize(n_vectors * n_dimensions);
    unsigned int* d_output =   thrust::raw_pointer_cast(&*d_output_device.begin());


    CUT_CHECK_ERR("start of GPU Sobol\n");



    sobolGPUInts(n_vectors, n_dimensions,path_offset, d_directions, d_output);
    cutilSafeCall(cudaThreadSynchronize());
    CUT_CHECK_ERR("end of GPU Sobol\n");
 
        return 0;
    
}

int SobDeviceMain(int n_vectors, int n_dimensions, unsigned path_offset, thrust::host_vector<float>& h_outputGPU_vec, bool doInverseNormal )
{
    // We will generate n_vectors vectors of n_dimensions numbers

   CUT_CHECK_ERR("start of Sobol \n");
   
    thrust::device_vector<float> dev_output(n_vectors * n_dimensions);

    int res =  SobDevice( n_vectors, n_dimensions, path_offset, dev_output, doInverseNormal);

    std::cout << "Reading results from GPU..." << std::endl;

    h_outputGPU_vec = dev_output;
