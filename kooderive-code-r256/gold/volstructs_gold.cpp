												  ...
												  )
{
	...
	for (int step=0; step < steps; ++step)
	{
		...
		pseudoSqrtSchur(covariance, pseudoRoots[step],  factors, true );
	}
												  ...
												  )
{
	...
	MatrixConstFacade<double> corrFacade(correlationMatrix);
...
Matrix_gold<double> GenerateConstantCorrelationMatrix(int stocks,
											  double rho)
{
	int numberRates = stocks;
	Matrix_gold<double> res(numberRates,numberRates,0.0);
...
Cube_gold<double> FlatVolConstCorrelationPseudoRoots(double sigma, 
								  const std::vector<double>& evolutionTimes,
								  int stocks,
								  int factors,
								  double rho)
{
	...
	int numberRates = stocks;
	int steps =  static_cast<int>(evolutionTimes.size());
	...
	for (int step=0; step < steps; ++step)
	{
		...
		for (int r=0; r < numberRates; ++r)
		{
            ...
            double var=time*sigma*sigma;
            double covsr = rho*var;
			covariance(r,r) = var;
