...
template<class D>
class earlyExerciseCallGold
{
public:
    earlyExerciseCallGold(const std::vector<D>& floatingInputs,
        const std::vector<int>& integerInputs, 
        int numberOfSteps) :strike_(floatingInputs[0])
    {}

     earlyExerciseCallGold()
    {}
    ...
    D exerciseValue(int stepEvolution,
        ...
        const CubeConstFacade<D>& allDiscountRatios)
    {   
        return std::max(rate1-strike_,0.0);
    }
