...
void OneDBSMonteCarloGreekDoubleDigitals(int paths, int verbose,  int numberFlows,  Realv K1, Realv K2, 
                                         Realv S0,
                                         Realv sigma,
                                         Realv r,
                                         Realv T,
...
int BSGreeksDDmain()
{
    ...
    std::cout << "S0,"<<  S0 << ",Sigma,"<< sigma<< ",r,"<< r<<",d,"<< d<<",T,"<< T<<",K1,"<< K1<<",K2,"<< K2 << "\n";

    int N=9;
    Matrix_gold<Realv> outputData(N+1,dataPoints,-1.0);

    int n_steps=1;
    for (int j=1; j<=N; ++j,n_steps*=2)
    {

        MonteCarloStatisticsSimple stats(names.size(),names);
        OneDBSMonteCarloGreekDoubleDigitals(16383, false, 1,  K1, K2, S0,
            sigma,
            r,
            T,
            d,
            n_steps,stats);
        {
            std::vector<std::vector<Realv> > output(stats.GetStatistics());
            std::vector<std::string> names(stats.GetStatisticsNames());
            ...
            for (size_t m=0; m < output[1].size(); ++m)
                {
                    outputData(j,m) =output[1][m];
                }
