...
namespace
{
    const Realv A[4]={0.3253030, 0.4211071, 0.1334425, 0.006374323}; 

    const Realv B[4]={0.1337764, 0.6243247, 1.3425378, 2.2626645  };



    inline Realv f(Realv x, Realv y, Realv aprime, Realv bprime, Realv rho) 
    {
        Realv r = aprime*(2*x-aprime) + bprime*(2*y-bprime) + 2*rho*(x-aprime)*(y-bprime);
        return exp(r);
    }
}
// from Hull 3rd edition page 260 based on Drezner Computation of the Bivariate Normal Integral
// Mathematics of computation 21 277--279

Realv BivariateCumulativeNormalDrezner(Realv a, Realv b, Realv rho) 
{
    if ( (a<=0.0) && (b<=0.0) && (rho<=0.0) ) 
    { 
        Realv aprime = a/std::sqrt(2.0*(1.0-rho*rho));
        Realv bprime = b/std::sqrt(2.0*(1.0-rho*rho));
        Realv sum = 0;
        for (int i=0;i < 4; i++) 
        {
            for (int j=0; j < 4; j++) 
        ...
        Realv M = sum * ( std::sqrt(1.0-rho*rho)/M_PI);
        return M;
    }
    else  
        if ( a * b * rho <= 0.0 ) 
        {
            if ( ( a<=0.0 ) && ( b>=0.0 ) && (rho>=0.0) ) 
            {
                return cumulativeNormal(a) - BivariateCumulativeNormalDrezner(a, -b, -rho); 
            }
            else 
                if ( (a>=0.0) && (b<=0.0) && (rho>=0.0) ) 
                {
                    return cumulativeNormal(b) - BivariateCumulativeNormalDrezner(-a, b, -rho); 
                }
                else 
                    if ( (a>=0.0) && (b>=0.0) && (rho<=0.0) ) 
                    {
                        return cumulativeNormal(a) + cumulativeNormal(b) - 1.0 + BivariateCumulativeNormalDrezner(-a, -b, rho); 
                    }
        }
        else  
            if ( a * b * rho >= 0.0 ) 
            {
                Realv denominator = sqrt(a*a - 2*rho*a*b + b*b);
                Realv rho1 = ((rho * a - b) * sgn(a))/denominator;
                Realv rho2 = ((rho * b - a) * sgn(b))/denominator;
                Realv delta=(1.0-sgn(a)*sgn(b))/4.0;
                return BivariateCumulativeNormalDrezner(a,0.0,rho1) + BivariateCumulativeNormalDrezner(b,0.0,rho2) - delta;
            }

            GenerateError("logic failure:");
}