...
#include <gold/sobol_primitives.h>
...
namespace
{
    void sobolCPUints(int n_vectors, int n_dimensions, unsigned int *directions, unsigned int *output)
    {
        unsigned int *v = directions;
...
void sobolCPUintsOffset(int n_vectors, int n_dimensions, int pathOffset, unsigned int *directions, unsigned int *output)
{
    ++pathOffset;

    std::vector<unsigned int> allVars((n_vectors+pathOffset)*n_dimensions);
    sobolCPUints(n_vectors+pathOffset, n_dimensions, directions,&allVars[0]);

    for (int d=0; d < n_dimensions; ++d)
        for (int i=0; i < n_vectors; ++i)   
        {
            output[i + n_vectors * d] = allVars[i +pathOffset+ (n_vectors+pathOffset) * d];
        }
...
void sobolCPU(int n_vectors, int n_dimensions, unsigned int *directions, double *output)
{
    unsigned int *v = directions;
    ...
    for (int d = 0 ; d < n_dimensions ; d++)
    {
        ...
        for (int i = 1 ; i < n_vectors ; i++)
        {
            ...
            // In the Bratley and Fox paper this is equation (**)
            X ^= v[ffs(~(i - 1)) - 1];
            output[i + n_vectors * d] = (double)X * k_2powneg32;
        }
        v += n_directions;
    }
}
